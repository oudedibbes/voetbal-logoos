	...
	return gulp
		...
		.pipe(
			imagemin([
				imagemin.svgo({
					multipass: false,
					plugins: [{ removeUselessDefs: false }, { removeHiddenElems: false }, { removeViewBox: false }, { cleanupIDs: false }],
				}),
			])
		)
...
const build = gulp.series(gulp.parallel(css, images, fonts, html, js, vendors, polyfills))

const buildall = gulp.series(clean, gulp.parallel(css, images, favicon, fonts, html, js, vendors))
